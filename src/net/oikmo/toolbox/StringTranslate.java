...
public class StringTranslate {
	...
	private StringTranslate() {
		try {
			...
			this.translateTable.load(new FileInputStream(Main.getDir().getAbsolutePath()+"/options.save"));
	...
	public void insertKey(String name, String value) {
		...
		try {
			this.translateTable.store(new FileOutputStream(Main.getDir().getAbsolutePath()+"/options.save"), null);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
